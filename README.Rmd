...
A second drones dataset called `lens_drones` contains 9,071 patent documents from the Lens patent database that refer to the word drone or drones somewhere in the text. This dataset consists of a single family representative of 16,419 patent publications returned by the search of the full text. This data will be of interest to researchers wishing to work exclusively with open access databases. A third dataset `patsnap_family` from the [Patsnap](http://www.patsnap.com/) commercial database is intended for use in patent family analysis and is based on a search of patent claims. Additional datasets will be added over time.

The data within the drones package is intended exclusively for use in training in patent analytics as described on the [WIPO analytics](https://github.com/wipo-analytics) home page. It is important to note that the dataset is deliberately noisy in order to allow support work on methods for cleaning data. While this is valid data, it is not intended to be used to make definitive statements about patent activity for drone technology and it is not expected to be complete. The drones patent dataset is part of work in progress on the WIPO Patent Analytics Handbook. 
